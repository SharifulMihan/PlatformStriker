...
public class LevelTransitionManager : MonoBehaviour
{
    ...
    [Header("Panels")]
    [Tooltip("Shown first — slides in from the left showing the door's 'level complete' image.")]
    ...
    /// <summary>Call from Door.cs once the level is complete. Each door passes its own
    /// scene-specific sprites, so the shared persistent Canvas always shows the right art.</summary>
    ...
    // Null sprite = the Door's inspector field was never assigned in this scene. Warn loudly
    // and hide the image, instead of silently keeping whatever the previous level showed —
    // that silent-stale behavior is exactly the bug this system had with text.
    private void SetSprite(Image image, Sprite sprite)
    {
        if (image == null) return;

        if (sprite == null)
            Debug.LogWarning("[LevelTransitionManager] No sprite assigned — fill in the Door's " +
                             "Complete Sprite / Next Level Sprite fields in scene '" +
